...
function orderLookup(query){
    ...
    if(!checkUserInput(query)){
        return false;
    }
    var poNumber="";var material="";
    var orderList = sap.ui.getCore().byId("orderList");
    busyDialog.open();
    var orderModel = new sap.ui.model.json.JSONModel();
    if(sap.ui.getCore().byId("poPNSwitch").getState()){
        poNumber=query.toUpperCase();
        material=" ";
    }else{
        poNumber=" ";
        material=query.toUpperCase();
    }
    orderModel.loadData(getDataEndPoint("orderModel",null,poNumber,material));//material
