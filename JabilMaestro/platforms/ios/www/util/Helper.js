...
function getDataEndPoint(datapoint, inputDate,inputVar,inputVar1){
    ...
    switch(datapoint){
        ...
        case "orderModel" :authenticate(false);
            return  url + "{\"service\":\"orderStatus.xsodata\",\"poNumber\":\""+inputVar+"\",\"material\":\""+inputVar1+"\",\"customer\":\""+userCustomer+"\",\"format\":\"json\"}"
...
function checkUserInput(input) {
    var pattern = new RegExp(ALPHA_EXPRESSION_SPECIAL_CHAR);
    if(input != undefined && $.trim(input) != ""){
        if(!pattern.test($.trim(input))){
            sap.m.MessageBox.alert(i18nModel.getResourceBundle().getText("InputValidationError"));
            return false;
        }
        return true;
    }
