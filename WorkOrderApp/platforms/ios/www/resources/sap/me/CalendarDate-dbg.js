...
sap.ui.define(['jquery.sap.global'],
    function (jQuery) {
        ...
        theCalendarDate.prototype.getCopyDateObject = function () {
            ...
            // TODO: remove this!
            if (cloneDate.getFullYear() !== this._date.getFullYear()
                || cloneDate.getMonth() !== this._date.getMonth()
                || cloneDate.getDate() !== this._date.getDate()
                || cloneDate.getHours() !== this._date.getHours()
                || cloneDate.getMinutes() !== this._date.getMinutes()
                || cloneDate.getSeconds() !== this._date.getSeconds()
                ) {
                jQuery.log.error("Cloned date not identical to date.");
                jQuery.log.error("Clone: " + cloneDate);
                jQuery.log.error("Date:  " + this._date);
            }
            return cloneDate;
        };
        ...
        theCalendarDate.getMonthFromString = function (strMonth) {
            ...
            // qUnits fail on IE11 because of unsupported function 'indexOf', as in "aMonths.indexOf(strMonth)"
            var i;
            var index = -1;
            for (i = 0; i < aMonths.length; i++) {
                if (aMonths[i] === strMonth) {
                    index = i;
                    break;
                }
            }
