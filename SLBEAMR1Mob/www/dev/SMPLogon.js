 ...
 ********************************************************************/
function doLogonInit() {
  //Init needs to happen before anything else.
    console.log("Entering doLogonInit");
    jQuery.sap.require("sap.ui.thirdparty.datajs");
    $.getJSON( "project.json", function( data ) {
        if(data && data.hybrid) {
              app.smpInfo.server = data.hybrid.server;
              app.smpInfo.port = data.hybrid.port;
              app.smpInfo.appID = data.hybrid.appid;
              }
        if (app.smpInfo.server && app.smpInfo.server.length > 0) {
           var context = {
                "serverHost" : app.smpInfo.server,
                "https" : "false",
                "serverPort" : app.smpInfo.port,
                "user": "IMohammed6",
                "password": "itt123",
                "serverPort" : app.smpInfo.port,
                "passcode": "password",
              ...
              //register user in SMP
              sap.Logon.init(onLogonInitSuccess, onLogonError, app.smpInfo.appID, context);
              }
            });
