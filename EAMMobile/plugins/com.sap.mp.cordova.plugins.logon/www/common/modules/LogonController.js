        ...
        //before full registration callback is called.
            this.postMatchAction = function(matchedRule){
               if (matchedRule.action == onFullRegistered ){
                   ...
                   else if (bUnlockPerformed || this.forceSAMLAuth) {  //no need to do saml authentication again if data vault is already unlocked when unlock method is called, unless the call is started by logon.init
        ...
        var wrapCallbackWithQueueNext = function(callback) {
            return function() { 
				...
				try{
                	...
                	alert("Appliction callback exception: " + JSON.stringify(ex));
				}
             	if (flowqueue) {
