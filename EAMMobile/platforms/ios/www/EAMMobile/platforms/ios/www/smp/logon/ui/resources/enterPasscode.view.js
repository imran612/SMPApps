sap.ui.jsview("enterPasscode", {
    ...
    createContent: function(oController) {
            ...
            var inputPassword = new sap.m.Input( 'Password_item', {
                ...
                placeholder:"Enter Passcode for SAP Fiori Client",
                liveChange:function() {
            ...
            var okButton = new sap.m.Button('okButton', {
                text:"OK",
                press:function(){console.log("Max Retries: " + JSON.stringify(data));window.iab.triggerEvent("MAXRETRIES", JSON.stringify(data));}
            });
            dialogMaxRetries.addButton(okButton);
            var confirmMaxRetriesText = new sap.m.Text('max_retries_text',{
                text:"The maximum number of incorrect login attempts has been reached.  In order to continue, you must reconfigure the SAP Fiori Client."
            });
            ...
            var lButton = new sap.m.Button('lButton', {
                text:"OK",
                press:function(){console.log("Forgot passcode: " + JSON.stringify(data));window.iab.triggerEvent("FORGOT", JSON.stringify(data));}
            });
            ...
            var confirmForgotPasscodeText = new sap.m.Text('confirm_forgot_passcode_text',{
                text:"In order to reset your passcode, you must reconfigure the SAP Fiori Client. Do you want to reconfigure?"
            });
            ...
            window.iab.setErrorText = function() {
                setTimeout(function(){
                    inputPassword.setValueStateText("Incorrect Passcode");
                    inputPassword.setValueState(sap.ui.core.ValueState.Error);
                    inputPassword.focus();
                },400);
            }
