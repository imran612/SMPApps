...
- (void)show:(CDVInvokedUrlCommand*)command
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        static BOOL animated = YES;  //kapsel customization: only first time start to use animation to avoid ui deadlock
        if (self.inAppBrowserViewController != nil) {
            [self.viewController presentModalViewController:nav animated:animated];
            animated = NO;
        }
    });
}
...
- (void)close
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([self respondsToSelector:@selector(presentingViewController)]) {
            [[self presentingViewController] dismissViewControllerAnimated:NO completion:nil];  //Kapsel customization: Change Animated to NO to avoid UI dead lock 
        } else {
            [[self parentViewController] dismissModalViewControllerAnimated:NO];  //Kapsel customization: Change Animated to NO to avoid UI dead lock
        }
    });
}
