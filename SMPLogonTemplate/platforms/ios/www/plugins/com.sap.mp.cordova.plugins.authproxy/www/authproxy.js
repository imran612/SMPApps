cordova.define("com.sap.mp.cordova.plugins.authproxy.AuthProxy", function(require, exports, module) { // 3.6.6
 ...
 */
AuthProxy.prototype.sendRequest2 = function (method, url, header, requestBody, successCB, errorCB, timeout, authConfig) {
    ...
    try {
        var client = new Client2(method, url, header, requestBody, successCB, errorCB, timeout, authConfig);
        return client.send();
    } catch (ex) {
        errorCB({
...
var Client2 = function (method, url, header, requestBody, successCB, errorCB, timeout, authConfig) {
    ...
    this.send = function () {
        ...
        var args = [this.Method, this.Url, this.Header, this.RequestBody, this.Timeout, this.authConfig];
...
AuthProxy.prototype.isInterceptingRequests = function(successCallback, errorCallback) {
    // The native function should only be invoked on Android
    if (device.platform.toLowerCase().indexOf("android") >= 0) {
        exec(successCallback, errorCallback, "AuthProxy", "isInterceptingRequests", []);
    } else {
...
// Only implemented on Android.
AuthProxy.prototype.startInterceptingIfNeeded = function(successCallback, errorCallback) {
    // The native function should only be invoked on Android
    if (device.platform.toLowerCase().indexOf("android") >= 0) {
