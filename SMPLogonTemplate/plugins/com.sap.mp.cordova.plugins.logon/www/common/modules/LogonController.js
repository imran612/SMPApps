    ...
    var getAppConfig = function(context){
        ...
        if (context.operation.currentConfigState =="complete" || context.operation.currentConfigIndex == context.operation.configSources.length && context.operation.configState == "done"){
       ...
       if (context.operation.currentConfigState == "run"){
           ...
           */ else if (configType == "user"){
                ...
                connsole.log("unknown config type: " + configType);
            }
    ...
    var startLogonInit = function(context){
        ...
        if ( device.platform.toLowerCase() == "ios" ){
            if (context.appConfig.certificate == "afaria" ){
                sap.logon.Core.useAfaria(context.appConfig.appID, context.smpConfig, logon, appDelegate.onRegistrationError, true);
            }
            else{
                sap.logon.Core.useAfaria(context.appConfig.appID, context.smpConfig, logon, appDelegate.onRegistrationError, false);
            }
        }
        else{
            if (context.appConfig.certificate == "afaria" ){
        ...
        function logon(){
            ...
            if (context.appConfig.isForSMP){
               sap.Logon.init(function(m) {appDelegate.onRegistrationSuccess(m); navigator.splashscreen.hide();}, function(m) {appDelegate.onRegistrationError(m); navigator.splashscreen.hide();}, context.appConfig.appID, context.smpConfig, context.operation.logonView, context.appConfig.x509CertProvider);
            }
            else{
               ...
               sap.Logon.initPasscodeManager(function(m){appDelegate.onRegistrationSuccess(m); navigator.splashscreen.hide();}, function(m) {appDelegate.onRegistrationError(m); navigator.splashscreen.hide();}, context.appConfig.appID, context.operation.logonView, context.appConfig.passcodePolicy, context.smpConfig);
            }
        }
    };
            ...
            for (var i=0; i<inputContext.auth.length; i++){
               if (inputContext.auth[i]["type"] == "saml2.web.post"){
                   ...
                   if (inputContext.auth[i]["config"]["saml2.web.post.finish.endpoint.uri"].substring(0, 4).toLowerCase()!="http"){
                       ...
                       inputContext.auth[i]["config"]["saml2.web.post.finish.endpoint.uri"] = path+'://'+inputContext.serverHost+':'+inputContext.serverPort+inputContext.auth[i]["config"]["saml2.web.post.finish.endpoint.uri"];
        ...
        var initSuccess = function(certificateSetToLogonCore){
            ...
            registerOrUnlock(function(context) {
                ...
                var url = (context.registrationContext.https ? "https" : "http") + "://" 
                    ...
                    + (isDefault(context.registrationContext.serverPort) ? "" : (":" + context.registrationContext.serverPort))
                    + "/";
						...
						if (result.status == 200 && result.responseText) {
							...
							if (config.host && config.port && config.protocol) {
                               ...
                               if (getCertificateProviderConfig(config) != null){
									...
									_oLogonCore.setUserCreationPolicy(successOrError, successOrError, "certificate", applicationId, _providedContext );
								} else {
									_oLogonCore.initLogon(initSuccessMobilePlace, initError, applicationId, _credentialProviderID);
								}
            ...
            if ( credentialProviderID ){
               ...
               var isRegisteredCallback = function(result) {
                   if (result) {
                        ...
                        _oLogonCore.setUserCreationPolicy(callbackMethod, callbackMethod, "certificate", applicationId, _providedContext );
                   }
               }
               _oLogonCore.isRegistered(isRegisteredCallback, initError, applicationId);
    ...
    var initPasscodeManager = function (successCallback, errorCallback, applicationId, customView, passcodePolicy, context ) {
      ...
      _oLogonCore.initLogon(initSuccess, initError, applicationId, null, _bIsWebRegistration );
    ...
    var FlowRunnerQueue = function() {
        ...
        //wrapped into a function to defer evaluation of the references to flow callbacks
            ...
            var startRegistration = function(onSuccess, onError, context){
                ...
                if ((cordova.require("cordova/platform").id.indexOf("windows") === 0) && getSAMLConfig(_providedContext) != null) {
                    var errorText = 'SAML Registration is not supported on the Windows 8.1 / Windows Phone 8.1 platforms. Please use a different method of registering with the SMP server.';
                    utils.log(errorText);
                    if (typeof ("Windows" !== 'undefined')) {
                        (new Windows.UI.Popups.MessageDialog(errorText, "Unsupported Feature")).showAsync().done();
                    }
                    var error = { 
                        errorDomain : "MAFLogonCoreErrorDomain",
                        errorCode: -1
                    };
                    onError(error);
                }
        ...
        //before full registration callback is called.
            this.postMatchAction = function(matchedRule){
               if (matchedRule.action == onFullRegistered ){
                   ...
                   else if ((bUnlockPerformed || this.forceSAMLAuth)&&window.location.href.toLowerCase().indexOf("file:")==0) {
...
// === flow launcher methods =====================================
      ...
      var deletePasscodeManager = function(onsuccess, onerror) {
		...
		function mergeSettingsIntoRegistrationContext(settings, context) {
			...
			var propertiesToMerge = ["serverHost","serverPort","https","auth"];
