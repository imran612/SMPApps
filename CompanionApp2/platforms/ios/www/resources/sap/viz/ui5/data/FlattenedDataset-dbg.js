...
sap.viz.ui5.data.FlattenedDataset.prototype._createVIZFlatTable = function() {
	...
	if ( !aContexts || aContexts.length === 0 ) {
		if ( this._oVIZFlatTable ) {
			delete this._oVIZFlatTable;
			this.invalidate();
		}
		return;
	}
	...
	jQuery.each(this.getDimensions(), function(i,oColumn) {
		aAxis.push({def: oColumn, adapter : oColumn._getAdapter()});
...
sap.viz.ui5.data.FlattenedDataset.prototype._createVIZCrosstab = function() {
	...
	if ( !aContexts || aContexts.length === 0 ) {
		if ( this._oVIZCrosstab ) {
			...
			delete this._aCrossContextLookup;
			delete this._aFlatContextLookup;
			this.invalidate();
		}
		return;
	}
	...
	// transform dimension definitions into a structure more suitable for our transformation
	jQuery.each(this.getDimensions(), function(i,oColumn) {
	...
	// is returned. If not, the tuple is added at the end and the new index is returned.
	function getAxisIndex(aAxisDef, aAxisValues, oContext) {
		...
		// TODO PERFOPT: search is only needed when more than one axis is defined and only because it is 
		// not clear in what order the combinations of axis values are enumerated in the model. 
		// It could be x1y1,x2y1,..xNy1,x1y2,... or x1y1,x1y2,...x1yM,x2y1,... or totally unsorted.
	...
	jQuery.each(aContexts, function(iIndex, oContext) {
		...
		// 2. only one axis is defined, use the getAxisIndex without search + a fixed 0 for the second axis
		// 3. if no axes are defined, use an incrementing index for the first axis and 0 for the second
		// The decision between the three cases can be taken outside the loop.
...
sap.viz.ui5.data.FlattenedDataset.prototype.findContext = function(oCriteria) {
	if ( this._aCrossContextLookup && oCriteria.dii_a2 != undefined && oCriteria.dii_a1 != undefined && typeof oCriteria === 'object' ) {
		return this._aCrossContextLookup[oCriteria.dii_a2] && this._aCrossContextLookup[oCriteria.dii_a2][oCriteria.dii_a1];
	} else if ( this._aFlatContextLookup && oCriteria._id !== undefined && typeof oCriteria === 'object' ) {
		return this._aFlatContextLookup[oCriteria._id]
	}
