cordova.define("com.sap.mp.cordova.plugins.logon.LogonJsView", function(require, exports, module) {     var utils = sap.logon.Utils;
        ...
        if (screenId == "SCR_SAML_AUTH"){
             ...
             if (device.platform == 'iOS') {
                ...
                var sendSAMLRequest = function() {
                    var payload = "window.location=\"" + endpointUrl + "\"";
                    // SAML against an SMP server requires the first request to have the application id to set
                    // a proper X-SMP-SESSID cookie. The url constructed as below works. This extra request
                    // has no effect against HMC.
                ...
                sap.AuthProxy.isInterceptingRequests(function(isInterceptingRequests) {
                    // Requests from the inAppBrowser won't get intercepted on older devices.
                    // Make sure we only switch to https if the request will be intercepted.
                    var versionToCompare = device.version;
                    if (versionToCompare.length > 3) {
                        // Get the first two numbers in the version
                        // eg: from 4.4.4 get 4.4, or from 5.0.1 get 5.0
                        versionToCompare = device.version.substring(0,2);
                    }
                    var versionAtLeast44 = parseFloat(versionToCompare) > 4.3;
                    if (isInterceptingRequests && endpointUrl.toLowerCase().indexOf("https") == 0 && versionAtLeast44){
